...
# etc_str -> 추가 필요한 정보 있으면 넣을 스트링형 데이타 (현재 사용 안함 필요하면 쓰세용!) 
    ...
    if data['type'] == "market":
        if data['side'] == "long":
            ...
            print(binanceX.create_order(data['ticker'], 'market', 'buy', data['amt']))

        elif data['side'] == "short":
            #print(binanceX.create_market_sell_order(data['ticker'], data['amt']))
            print(binanceX.create_order(data['ticker'], 'market', 'sell', data['amt']))

    elif data['type'] == "limit":
        if data['side'] == "long":
            #print(binanceX.create_limit_buy_order(data['ticker'], data['amt'], data['price_money']))
            print(binanceX.create_order(data['ticker'], 'limit', 'buy', data['amt'], data['price_money']))
        elif data['side'] == "short":
            #print(binanceX.create_limit_sell_order(data['ticker'], data['amt'], data['price_money']))
            print(binanceX.create_order(data['ticker'], 'limit', 'sell', data['amt'], data['price_money']))
    ...
    if data['type'] == "market":
        ...
        if data['etc_str'] == "open":
            ...
            if data['side'] == "long":
                ...
                print(bybitX.create_order(data['ticker'], 'market', 'buy', data['amt']))
                
            elif data['side'] == "short":
                #print(bybitX.create_market_sell_order(data['ticker'], data['amt']))
                print(bybitX.create_order(data['ticker'], 'market', 'sell', data['amt']))

        elif data['etc_str'] == "close":
            
            if data['side'] == "long":
                #print(bybitX.create_market_buy_order(data['ticker'], data['amt'],{'reduce_only': True,'close_on_trigger':True}))
                print(bybitX.create_order(data['ticker'], 'market', 'buy', data['amt'], None,{'reduce_only': True,'close_on_trigger':True}))
            elif data['side'] == "short":
                #print(bybitX.create_market_sell_order(data['ticker'], data['amt'],{'reduce_only': True,'close_on_trigger':True}))
                print(bybitX.create_order(data['ticker'], 'market', 'sell', data['amt'], None, {'reduce_only': True,'close_on_trigger':True}))
        ...
        if data['etc_str'] == "open":
            ...
            if data['side'] == "long":
                ...
                print(bybitX.create_order(data['ticker'], 'limit', 'buy', data['amt'], data['price_money']))
            elif data['side'] == "short":
                #print(bybitX.create_limit_sell_order(data['ticker'], data['amt'], data['price_money']))
                print(bybitX.create_order(data['ticker'], 'limit', 'sell', data['amt'], data['price_money']))
            ...
            if data['side'] == "long":
                ...
                print(bybitX.create_order(data['ticker'], 'limit', 'buy', data['amt'], data['price_money'] ,{'reduce_only': True,'close_on_trigger':True}))
            elif data['side'] == "short":
                #print(bybitX.create_limit_sell_order(data['ticker'], data['amt'], data['price_money'],{'reduce_only': True,'close_on_trigger':True}))
                print(bybitX.create_order(data['ticker'], 'limit', 'sell', data['amt'], data['price_money'] ,{'reduce_only': True,'close_on_trigger':True}))
