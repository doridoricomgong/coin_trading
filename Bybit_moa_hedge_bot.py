    ...
    try: 
        ...
        if abs(amt_b) > 0:
            ...
            if Rate > 0 and Rate != CoinPortion:
                    ...
                    if GapAmt >=  minimun_amount and abs(GapRate) >= (CoinPortion / 2.0): 
                        ...
                        if revenue_rate_b >= Target_Revenue_Rate:
                            ...
                            if abs(amt_b) >= GapAmt * 3.0:
                                ...
                                print(bybitX.create_order(Target_Coin_Ticker, 'market', 'sell', GapAmt, None,{'reduce_only': True,'close_on_trigger':True}))
                ...
                #갭이 양수면 비트코인 비중이 적으니 추매할 필요가 있는 경우
                    ...
                    if GapAmt >=  minimun_amount  and abs(GapRate) >= (CoinPortion / 10.0):
                        ...
                        print(bybitX.create_order(Target_Coin_Ticker, 'market', 'buy', GapAmt))
            ...
            if CoinPortion > 0:
                ...
                print(bybitX.create_order(Target_Coin_Ticker, 'market', 'buy', BestAmt))
        ...
        if abs(amt_s) == 0 and hour == 0:
            ...
            #이전 캔들이 음봉이다!
            if df_day['open'][-2] > df_day['close'][-2]:


                #캔들 크기를 구합니다. 
                candle_rate = ((df_day['open'][-2] / df_day['close'][-2]) - 1.0) * 100

                if df_day['close'][-2] > df_day['open'][-2]:
                    candle_rate = ((df_day['close'][-2] / df_day['open'][-2]) - 1.0) * 100
                ...
                print(bybitX.create_order(Target_Coin_Ticker, 'market', 'sell', hedge_amt))
