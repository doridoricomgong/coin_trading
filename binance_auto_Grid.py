...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
아래 내용이 복잡하고 이해가 힘들다면 이전 강의(챕터 9-3)의 깨끗한 코드(?)를 기반으로 수정해 나가시는 것도 방법입니다!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
for ticker in Tickers:
    ...
    try: 
        ...
        if "/USDT" in ticker:
            ...
            Max_Amt = float(binanceX.amount_to_precision(Target_Coin_Ticker, myBinance.GetAmount(float(balance['USDT']['total']),coin_price,Invest_Rate / CoinCnt)))  * leverage 
            ...
            if amt_s == 0:
                ...
                data = binanceX.create_order(Target_Coin_Ticker, 'market', 'sell', FirstAmt, None, params)
                ...
                ##################################################
                Adjust_Max_Amt = (data['amount'] - Buy_Amt)
                ...
                while TotalWater_Amt + Water_amt < Max_Water_Amt - Adjust_Max_Amt: ######여기서 최대 맥스수량을 보정해 줍니다#####
                    ...
                    print(binanceX.create_order(Target_Coin_Ticker, 'limit', 'sell', Water_amt, water_price, params))
                ...
                if abs(amt_s) > 0:
                    ...
                    for order in orders:
                        if order['status'] == "open" and order['info']['positionSide'] == "SHORT" and order['side'] == "sell":
                    ...
                    if Is_Water_Remain == False:
                        ...
                        #print(binanceX.create_market_buy_order(Target_Coin_Ticker,Cut_Amt,params))
                        print(binanceX.create_order(Target_Coin_Ticker, 'market', 'buy', Cut_Amt, None, params))
            ...
            if amt_b == 0:
                ...
                data = binanceX.create_order(Target_Coin_Ticker, 'market', 'buy', FirstAmt, None, params)
                ...
                while TotalWater_Amt + Water_amt < Max_Water_Amt - Adjust_Max_Amt: ######여기서 최대 맥스수량을 보정해 줍니다#####
                    ...
                    print(binanceX.create_order(Target_Coin_Ticker, 'limit', 'buy', Water_amt, water_price, params))
                ...
                if abs(amt_b) > 0:
                    ...
                    Is_Water_Remain = False
                    for order in orders:
                        if order['status'] == "open" and order['info']['positionSide'] == "LONG" and order['side'] == "buy":
                            Is_Water_Remain = True
                            break
                    ...
                    if Is_Water_Remain == False:
                        ...
                        #print(binanceX.create_market_sell_order(Target_Coin_Ticker,Cut_Amt,params))
                        print(binanceX.create_order(Target_Coin_Ticker, 'market', 'sell', Cut_Amt, None, params))
