        ...
        if "/USDT" in ticker:
            ...
            if myBybit.CheckCoinInList(TopCoinList,ticker) == True or myBybit.CheckCoinInList(DolPaCoinList,ticker) == True:
                ...
                for index in range(3,len(df)):
                    ...
                    #꺾인 지점을 체크한다 
                    if df['close'][-(index-1)] > df['close'][-(index)] < df['close'][-(index+1)]:
                        ...
                        if up_first_point == 0: #첫번째 꺾인 지점이 아직 셋팅 안되었다면 
                            ...
                            if coin_price > df['close'][-(index)]: #그 지점이 현재가보다 작다면 

                                #당첨!! 첫번째 꺾인 지점을 저장해 놓는다!
                                up_first_point = index #캔들 번호 
                                up_first_value = df['close'][-(index)] #캔들의 (종가)값
                            ...
                            if up_second_point == 0: # 두번째 꺾인 지점이 아직 셋팅 안되었다?
                                ...
                                if up_first_value > df['close'][-(index)]:

                                    #위 조건을 만족했다면 두번째 지점 셋팅!!
                                    up_second_point = index
                                    up_second_value = df['close'][-(index)]
                ...
                if abs(amt_s) == 0 and  UpTrendLineBefore < df['close'][-3] and UpTrendLine > df['close'][-2] and len(DolPaCoinList) < CoinCnt:
                ...
                for index in range(3,len(df)):
                    ...
                    if df['close'][-(index-1)] < df['close'][-(index)] > df['close'][-(index+1)]:
                        ...
                        if down_first_point == 0: #첫번째 꺾인 지점이 아직 셋팅 안되었다면 
                            ...
                            if coin_price < df['close'][-(index)]: #그 지점이 현재가보다 높다면 

                                #당첨!! 첫번째 꺾인 지점을 저장해 놓는다!
                                down_first_point = index #캔들 번호 
                                down_first_value = df['close'][-(index)] #캔들의 (종가)값
                            ...
                            if down_second_point == 0: # 두번째 꺾인 지점이 아직 셋팅 안되었다?
                                ...
                                if down_first_value < df['close'][-(index)]:

                                    #위 조건을 만족했다면 두번째 지점 셋팅!!
                                    down_second_point = index
                                    down_second_value = df['close'][-(index)]
                ...
                if abs(amt_b) == 0 and DownTrendLineBefore > df['close'][-3] and DownTrendLine < df['close'][-2] and len(DolPaCoinList) < CoinCnt:
                ...
                if IsDolPaLong == True and IsDolPaShort == False:
                    ...
                    print(bybitX.create_order(Target_Coin_Ticker, 'market', 'buy', Buy_Amt))
                    ...
                    print(bybitX.create_order(Target_Coin_Ticker, 'limit', 'sell', Buy_Amt, target_price,{'reduce_only': True,'close_on_trigger':True}))
                ...
                if IsDolPaShort == True and IsDolPaLong == False:
                    ...
                    print(bybitX.create_order(Target_Coin_Ticker, 'market', 'sell', Buy_Amt))
                    ...
                    print(bybitX.create_order(Target_Coin_Ticker, 'limit', 'buy', Buy_Amt, target_price,{'reduce_only': True,'close_on_trigger':True}))
