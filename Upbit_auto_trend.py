...
#OutCoinList = []
OutCoinList = ['KRW-BTC','KRW-ETH','KRW-ADA','KRW-DOT','KRW-AVAX','KRW-SOL','KRW-MATIC','KRW-ALGO','KRW-MANA','KRW-LINK','KRW-BAT','KRW-ATOM']
...
for ticker in Tickers:
    try: 
        ...
        if myUpbit.CheckCoinInList(TopCoinList,ticker) == True or myUpbit.CheckCoinInList(DolPaCoinList,ticker) == True:
            ...
            #추세선을 하향 돌파 했다! 
            if  UpTrendLineBefore < df['close'][-3] and UpTrendLine > df['close'][-2]:
            ...
            if DownTrendLineBefore > df['close'][-3] and DownTrendLine < df['close'][-2]:
